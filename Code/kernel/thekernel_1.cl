...
float sceneSDF(float3 ray, struct Ball ball)
{
  float sdf = INF;                                                                                  // Signed distance field.

  sdf = min(sdf, planeSDF(ray));                                                                    // Computing signed distance field...
  sdf = min(sdf, sphereSDF(ray, ball));                                                             // Computing signed distance field...
...
//////////////////////////////////////////// RAY MARCHING ////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////
float raymarch(float3 position, float3 direction, struct Ball ball)
{
  ...
  for (i = 0; i < MAX_STEPS; i++)
  {
    ...
    sdf = sceneSDF(ray, ball);                                                                            // Computing scene distance field...
...
float shadow(float3 position, float3 direction, struct Ball ball, struct Light light)
{
  float   distance = 0.0f;                                                                          // Marching distance.
  ...
  {
    ray = position + distance*direction;                                                            // Computing marching ray...   
    sdf_new = sceneSDF(ray, ball);                                                                  // Computing scene distance field... 
...
float3 normal(float3 position, struct Ball ball)
{
  ...
  sdf_L = sceneSDF(position - DX, ball);                                                                  // Computing signed distance field (left limit)...
  sdf_R = sceneSDF(position + DX, ball);                                                                  // Computing signed distance field (left limit)...
  nx = sdf_R - sdf_L;                                                                               // Computing gradient (x-component, centered derivative)...

  sdf_L = sceneSDF(position - DY, ball);                                                                  // Computing signed distance field (left limit)...
  sdf_R = sceneSDF(position + DY, ball);                                                                  // Computing signed distance field (left limit)...
  ny = sdf_R - sdf_L;                                                                               // Computing gradient (x-component, centered derivative)...

  sdf_L = sceneSDF(position - DZ, ball);                                                                  // Computing signed distance field (left limit)...
  sdf_R = sceneSDF(position + DZ, ball);                                                                  // Computing signed distance field (left limit)...
...
{ 
  ...
  camera.pos = (float3)(0.0f, 0.2f, 1.0f);                                                          // Setting initial camera position...
  ...
  ball.pos = ball_position[0].xyz;                                                                  // Setting ball position [x, y, z]...
  ball.rad = ball_position[0].w;                                                                    // Setting ball radius [radius]...
  ...
  d = raymarch(camera.pos, ray, ball);                                                                    // Computing scene distance...
  P = camera.pos + d*ray;                                                                           // Computing scene position...
  N = normal(P, ball);                                                                                    // Computing scene normal...
  ...
  diffusion = clamp(dot(N, incident), 0.0f, 1.0f)*shadow(P + N*2.0f*EPSILON, incident, ball, light);// Computing light diffusion intensity... 
  ...
  fragment_color[i + (uint)W*j] = (float4)(ray.x, ray.y, ray.z, 1.0f);
}
