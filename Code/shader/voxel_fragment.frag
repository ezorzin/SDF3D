...
void main()
{
  ...
  camera.pos = ((V_mat)*vec4(camera.pos, 1.0f)).xyz;                                         // Applying arcball to camera position...
  ...
  ray_temp = (V_mat*vec4(ray, 1.0f)).xyz;                                            // Applying arcball to camera direction...
  ray = normalize(ray_temp);
